...
// 
// Related Topics 设计 哈希表 数据流 有序集合 堆（优先队列） 👍 40 👎 0
...
class StockPrice {
    ...
    TreeMap<Integer,Integer> prices;  // 记录价格 和 价格出现的次数
    public StockPrice() {  // 初始化
        maxTimestamp = 0;
    ...
    public void update(int timestamp, int price) {
        maxTimestamp = Math.max(maxTimestamp,timestamp); // 获取最大时间戳
        int prevPrice = timePriceMap.getOrDefault(timestamp,0); //timePriceMap中有这个时间戳，返回对应的value,否则返回默认值0
        timePriceMap.put(timestamp,price); //将当前时间戳和对应的price 加入集合中
        if (prevPrice > 0) {
