...
// 每个服务器都直接或间接与别的服务器相连。 
// 
// Related Topics 广度优先搜索 图 数组 👍 42 👎 0
...
class Solution {
    public int networkBecomesIdle(int[][] edges, int[] patience) {
        // node to dist
        Map<Integer, Integer> nodeToDist = getNodeToDist(edges);  // todo

        // max time
        int ans = 0;
        for (int i = 1; i < patience.length; i++) {
            int dist = nodeToDist.get(i);
            int extra = (2 * dist -1) / patience[i];  //最多发送的次数
            int time = 2 * dist + extra * patience[i]; // 最后一次发送的时间 + 2*dist   (收到回复)
    ...
    private Map<Integer, Integer> getNodeToDist(int[][] edges) {
        ...
        Map<Integer, Set<Integer>> graph = getGraph(edges); // 邻接表
        // BFS
        // 1 init
        queue.offer(0);
        visited.add(0);
        ...
        while (!queue.isEmpty()) {
            ...
            while (size-- > 0) {
                ...
                for (int next : graph.getOrDefault(cur, new HashSet<>())) {
                    if (visited.add(next)) {  // 如果可以加入visited 说明未被访问可以加入队列
                        queue.offer(next);
                    }
                }
            }
