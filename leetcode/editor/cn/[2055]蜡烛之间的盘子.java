...
// 
// Related Topics 数组 字符串 二分查找 前缀和 👍 45 👎 0
...
class Solution {
    public int[] platesBetweenCandles(String s, int[][] queries) {
        // 前缀数组
        int[] nextCandleIdxs = getNextCandleIdxs(s);
        int[] prevCandleIdxs = getPrevCandleIdxs(s);
        ...
        int[] ans = new int[len];
        for(int i = 0 ; i < len ; i++) {
            int formerIdx = nextCandleIdxs[queries[i][0]];
            int latterIdx = prevCandleIdxs[queries[i][1]];
    ...
    private int[] getNextCandleIdxs(String s) {
        int len = s.length();
        int[] ans = new int[len];
    ...
    private int[] getPrevCandleIdxs(String s) {
        int len = s.length();
        int[] ans = new int[len];
