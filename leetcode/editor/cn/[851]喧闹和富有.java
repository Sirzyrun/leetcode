...
// 
// Related Topics 深度优先搜索 图 拓扑排序 数组 👍 91 👎 0
...
class Solution {
    public int[] loudAndRich(int[][] richer, int[] quiet) {
        ...
        for (int i = 0; i < n; i++) {
            list[i] = new ArrayList<Integer>(); //list[i]存储所有比personi 富有的人
        }
        for (int[] rich : richer) {
        ...
        for (int i = 0; i < n; i++) {
            dfs(i,quiet,answer,list);
        }
        return answer;
    }
    public void dfs(int x,int[] quiet,int[] answer,List<Integer>[] list) {
        if(answer[x] != -1){
            return;
        }
        answer[x] = x; // 比personx富有的人 首先确认为他自己 接着在比他富有的人中找到最安静的即quiet值最小的
        // 比personx富有的人 已经存储在list[x]中了 便在遍历这个数组在其中找到最安静的 并且递归因为其深度遍历之下的每一层都要比personx富有
        for (int y : list[x]){
            dfs(y,quiet,answer,list); // 比persony富有的人一定是比personx富有的 以此深度遍历下去
